...
void preprocessing(FILE *in, FILE *out) {
    ...
    while (fgets(buffer, BUFFER_SIZE, in) != NULL) { // 每次读取一行
        ...
        for (int i = 0; buffer[i] != '\0'; i++) {
            ...
            if (is_in_multiline_comments == 1 && buffer[i] != '\n') { // 如果在多行注释内部，将除了换行符的字符替换为空格
                buffer[i] = ' ';
            }

            if (is_in_multiline_comments == 0 && buffer[i] == '"') { // 处理字符串中的//
                is_in_string = !is_in_string; // 如果是双引号，则取反
            }

            if (is_in_multiline_comments == 0 & is_in_string == 0 & buffer[i + 1] != '\0' && buffer[i] == '/' &&
                buffer[i + 1] == '/') { // 如果检测到//并且不在多行注释与字符串内
                buffer[i] = '\n';
                buffer[i + 1] = '\0';
                break;
            }
